...
namespace ExplorationEngine.Containers
{
	public class Container_Ship
	{
		...
		public Container_Ship(BaseEntity BaseEnt, SolarSystem solarsystem)
		{
			Base = BaseEnt;
		...
		private void AddEntity(BaseEntity ent)
		{
		...
		private void RemoveEntity(BaseEntity ent)
		{
			if (Entities.Contains(ent))
			{
				Entities.Remove(ent);
		...
		private void RemoveEntities(List<BaseEntity> entities)
		{
			for (int ii = 0; ii < entities.Count; ii++)
			{
